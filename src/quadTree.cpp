...
const TreeNode& QuadTree::rootRefConst() const { return nodes[FIRST_LEVEL][FIRST_ID]; } //ссылка на (неизменяюему) корневую ноду
...
{
    //подсчет отсутствующих соседей (предполагается, что в этом дереве только одна нода)
    int ghostness = 0;
    for (auto n : Neighbours)
    {
