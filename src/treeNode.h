...
class TreeNode
{
    ...
    std::array<nodeEdgeId, EDGES_NUM> edges = { null, null, null, null, null, null, null, null }; //нумерация по часовой стрелке, начиная с левой половины верхней стороны
    //null по умолчанию для edges[EDGES_NUM] нужно укзаывать явно, т.к. nodeEdgeId по дефолту = 0
    ...
    Eigen::MatrixXd Jl[QUADRANTS_NUM]; //по одной для каждого подшаблона
...
public:
    ...
    bool isNodeInSubstencil(Quadrant q, const TreeNode& rnnode) const; //попадает ли ячейка в подшаблон для линейной функции
    ...
    std::string dumpEdgeVector(Edge etype) const; //дамп ребра ячейки в виде вектора
