...
void QuadTreeForest::meshRefineInitial()
{
    if (config.meshRefineAll)
    {
        for (auto depth = 1; depth < config.max_depth; depth++)
        {
            ...
            for (auto& rtree : trees) //проход по всем деревь€м по ссылке
            {
    ...
    for (auto depth = 1; depth < config.max_depth; depth++)
    {
        ...
        for (auto& tree : trees) //проход по всем деревь€м по ссылке
        {
            if (tree.isGhost()) //пропуск ghost-деревьев
                continue;
...
void QuadTreeForest::meshCoarsenInitial() //начальное склеивание сетки (дл€ теста)
{
